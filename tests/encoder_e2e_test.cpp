/// \file
/// \brief End-to-end tests for encoder-classifier and vision-detector runtime paths.
...
Tensor LoadVisionTensor(const cnpy::NpyArray& array)
{
...
void RunVisionDetectorE2ETest(const std::filesystem::path& artifact_dir)
{
    ...
    const Tensor image                 = LoadVisionTensor(sample.at("image"));
    const Tensor expected_sequence     = LoadVisionTensor(sample.at("sequence_output"));
    const Tensor expected_queries      = LoadVisionTensor(sample.at("query_features"));
    const Tensor expected_boxes        = LoadVisionTensor(sample.at("pred_boxes"));
    const Tensor expected_objectness   = LoadVisionTensor(sample.at("pred_objectness_logits"));
    const Tensor expected_class_logits = LoadVisionTensor(sample.at("pred_class_logits"));
...
int main(int argc, char** argv)
{
    try
    {
        if (argc == 3 && std::string(argv[1]) == "vision_detector_e2e")
        {
            RunVisionDetectorE2ETest(std::filesystem::path(argv[2]));
            return 0;
        }
        ...
        if (argc != 2)
        {
            throw std::runtime_error(
                "Usage: encoder_e2e_test <artifact-dir> "
                "[or encoder_e2e_test vision_detector_e2e <artifact-dir>]");
        }
        ...
        if (!std::filesystem::exists(artifact_dir / "artifact.json") || !std::filesystem::exists(sample_path))
        {
            std::printf("Skipping encoder e2e test: fixture files are not present.\n");
            return 0;
        }
        ...
        const auto                     load_status = runner.Load(bundle);
        Expect(load_status.ok(),
               "Encoder fixture should load through ModelRunner.");
        Expect(runner.HasEncoderClassifier(),
               "Encoder fixture should resolve to an encoder-classifier model.");
        ...
        Expect(logits.shape() == std::vector<std::int64_t>({1, 1}),
               "Encoder end-to-end logits shape mismatch.");
        ...
        Expect(std::fabs(actual - expected) < 5e-2F,
               "Encoder end-to-end logit mismatch. Expected "
